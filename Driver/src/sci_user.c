...
//void IRQ07_Handler(void) __attribute__((alias("uart3_interrupt_send")));
//#elif defined USE_SCI_SPI30
//void IRQ07_Handler(void) __attribute__((alias("spi30_interrupt")));
//#elif defined USE_SCI_IIC30
//void IRQ07_Handler(void) __attribute__((alias("iic30_interrupt")));
//#endif

//#if defined USE_SCI_UART3_RX
//void IRQ08_Handler(void) __attribute__((alias("uart3_interrupt_receive")));
//#elif defined USE_SCI_SPI31
//void IRQ08_Handler(void) __attribute__((alias("spi31_interrupt")));
//#elif defined USE_SCI_IIC31
//void IRQ08_Handler(void) __attribute__((alias("iic31_interrupt")));
//#endif
...
void spi30_interrupt(void)
{
    ...
    err_type = (uint8_t)(SCI2->SSR20 & _0001_SCI_OVERRUN_ERROR);
    SCI2->SIR20 = (uint16_t)err_type;
    ...
    else
    {
        if (g_spi30_tx_count > 0U || g_spi30_rx_count > 1U)
        {
            ...
            {
                *gp_spi30_rx_address = SCI2->SIO30;
                gp_spi30_rx_address++;
                g_spi30_rx_count--;
            }
            else
            {
                sio_dummy = SCI2->SIO30;
            }

            /* send data */
            if (0U != gp_spi30_tx_address)
            {
                SCI2->SIO30 = *gp_spi30_tx_address;
                gp_spi30_tx_address++;
                g_spi30_tx_count--;
            }
            else if((SCI2->SMR20 & _4000_SCI_CLOCK_MODE_TI0N) == 0)
            {
                SCI2->SIO30 = 0xFFU;   // dummy write in master reception mode
            }
        }
        else 
        {
            ...
            {
                while ((SCI2->SSR20 & _0020_SCI_VALID_STORED) == 0)
                {
                    ;
                }
                if (0U != gp_spi30_rx_address)
            ...
            if (0U != gp_spi30_tx_address)
            {
                if (((SCI2->SMR20 & _4000_SCI_CLOCK_MODE_TI0N) != 0) && ((SCI2->SCR20 & _4000_SCI_RECEPTION) != 0))
                {
                    ; /* Do not wait in slave transceiver mode */
                }
                else
                {
                    while (SCI2->SSR20 & _0040_SCI_UNDER_EXECUTE)
                    {
                        ;
                    }
                }
...
void spi31_interrupt(void)
{
    ...
    else
    {
        if (g_spi31_tx_count > 0U || g_spi31_rx_count > 1U)
        {
            ...
            {
                *gp_spi31_rx_address = SCI2->SIO31;
                gp_spi31_rx_address++;
                g_spi31_rx_count--;
            }
            else
            {
                sio_dummy = SCI2->SIO31;
            }

            /* send data */
            if (0U != gp_spi31_tx_address)
            {
                SCI2->SIO31 = *gp_spi31_tx_address;
                gp_spi31_tx_address++;
                g_spi31_tx_count--;
            }
            else if((SCI2->SMR21 & _4000_SCI_CLOCK_MODE_TI0N) == 0)
            {
                SCI2->SIO31 = 0xFFU;   // dummy write in master reception mode
            }
        }
        else 
        {
...
{
    if (((SCI2->SSR20 & _0002_SCI_PARITY_ERROR) == 0x0002U) && (g_iic30_tx_count != 0U))
    {
        iic30_callback_master_error(MD_NACK);
    }
    else if(((SCI2->SSR20 & _0001_SCI_OVERRUN_ERROR) == 0x0001U) && (g_iic30_tx_count != 0U))
    {
    ...
    else
    {
        ...
        if ((g_iic30_master_status_flag & _01_SCI_IIC_SEND_FLAG) == 1U)
        {
            ...
            {
                SCI2->SIO30 = *gp_iic30_tx_address;
                gp_iic30_tx_address++;
                g_iic30_tx_count--;
            }
            else
        ...
        else 
        {
            ...
            else
            {
                ...
                {
                    *gp_iic30_rx_address = SCI2->SIO30;
                    gp_iic30_rx_address++;
                    g_iic30_rx_count++;
                    
                    if (g_iic30_rx_count == (g_iic30_rx_length - 1U))
                    {
                        SCI2->SS2 &= (uint16_t)~_0001_SCI_CH0_OUTPUT_ENABLE;
                        SCI2->SIO30 = 0xFFU;
                    }
...
{
    if (((SCI2->SSR21 & _0002_SCI_PARITY_ERROR) == 0x0002U) && (g_iic31_tx_count != 0U))
    {
        iic31_callback_master_error(MD_NACK);
    }
    else if(((SCI2->SSR21 & _0001_SCI_OVERRUN_ERROR) == 0x0001U) && (g_iic31_tx_count != 0U))
    {
    ...
    else
    {
        ...
        if ((g_iic31_master_status_flag & _01_SCI_IIC_SEND_FLAG) == 1U)
        {
            ...
            {
                SCI2->SIO31 = *gp_iic31_tx_address;
                gp_iic31_tx_address++;
                g_iic31_tx_count--;
            }
            else
        ...
        else 
        {
            ...
            else
            {
                ...
                {
                    *gp_iic31_rx_address = SCI2->SIO31;
                    gp_iic31_rx_address++;
                    g_iic31_rx_count++;
